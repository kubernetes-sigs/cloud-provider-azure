...
func NewAuthProvider(armConfig *ARMClientConfig, config *AzureAuthConfig, clientOptionsMutFn ...func(option *policy.ClientOptions)) (*AuthProvider, error) {
	...
	if config.UseManagedIdentityExtension && len(config.AuxiliaryTokenSecretURI) > 0 && IsMultiTenant(armConfig) {
		networkTokenCredential, err = armauth.NewKeyVaultCredential(managedIdentityCredential, config.AuxiliaryTokenSecretURI)
		if err != nil {
			return nil, err
		}
	}
