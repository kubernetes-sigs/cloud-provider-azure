...
var resourceGroupName = "aks-cit-BlobServiceProperties"
...
var _ = ginkgo.BeforeSuite(func(ctx context.Context) {
	recorder, err = recording.NewRecorder("testdata/BlobServiceProperties")
	...
	realClient, err = New(subscriptionID, recorder.TokenCredential(), &arm.ClientOptions{
		ClientOptions: azcore.ClientOptions{
			...
			TracingProvider: utils.TracingProvider,
			Telemetry: policy.TelemetryOptions{
				ApplicationID: "ccm-BlobServiceProperties-client",
			},
		},
	})
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
