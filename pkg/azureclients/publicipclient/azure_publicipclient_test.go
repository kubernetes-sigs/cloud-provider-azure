...
const (
	testResourceID      = "/subscriptions/subscriptionID/resourceGroups/rg/providers/Microsoft.Network/publicIPAddresses/pip1"
	testVMSSResourceID  = "/subscriptions/subscriptionID/resourceGroups/rg/providers/Microsoft.Compute/virtualMachineScaleSets/vmss/virtualMachines/0/networkInterfaces/nic/ipconfigurations/ipConfig/publicipaddresses/pip1"
	resourceIDPrefix    = "/subscriptions/subscriptionID/resourceGroups/rg/providers/Microsoft.Network/publicIPAddresses"
	subResourceIDPrefix = "/subscriptions/subscriptionID/providers/Microsoft.Network/publicIPAddresses"
)
...
func TestNewAzureStack(t *testing.T) {
	...
	pipClient := New(config)
	assert.Equal(t, "AZURESTACKCLOUD", pipClient.cloudName)
	assert.Equal(t, "sub", pipClient.subscriptionID)
}
...
func TestListWithNextPage(t *testing.T) {
	...
	armClient := mockarmclient.NewMockInterface(ctrl)
	pipList := []network.PublicIPAddress{getTestPublicIPAddress("pip1"), getTestPublicIPAddress("pip2"), getTestPublicIPAddress("pip3")}
	partialResponse, err := json.Marshal(network.PublicIPAddressListResult{Value: &pipList, NextLink: ptr.To("nextLink")})
	assert.NoError(t, err)
...
func TestCreateOrUpdate(t *testing.T) {
	...
	armClient.EXPECT().PutResource(gomock.Any(), ptr.Deref(pip.ID, ""), pip).Return(response, nil).Times(1)
	armClient.EXPECT().CloseResponse(gomock.Any(), gomock.Any()).Times(1)
...
func TestCreateOrUpdateThrottle(t *testing.T) {
	...
	pip := getTestPublicIPAddress("pip1")
	armClient := mockarmclient.NewMockInterface(ctrl)
	armClient.EXPECT().PutResource(gomock.Any(), ptr.Deref(pip.ID, ""), pip).Return(response, throttleErr).Times(1)
	armClient.EXPECT().CloseResponse(gomock.Any(), gomock.Any()).Times(1)
...
func TestDelete(t *testing.T) {
	...
	pip := getTestPublicIPAddress("pip1")
	armClient := mockarmclient.NewMockInterface(ctrl)
	armClient.EXPECT().DeleteResource(gomock.Any(), ptr.Deref(pip.ID, "")).Return(nil).Times(1)
...
func TestDeleteThrottle(t *testing.T) {
	...
	pip := getTestPublicIPAddress("pip1")
	armClient := mockarmclient.NewMockInterface(ctrl)
	armClient.EXPECT().DeleteResource(gomock.Any(), ptr.Deref(pip.ID, "")).Return(throttleErr).Times(1)
...
func getTestPublicIPAddress(name string) network.PublicIPAddress {
	return network.PublicIPAddress{
		ID:       ptr.To(fmt.Sprintf("/subscriptions/subscriptionID/resourceGroups/rg/providers/Microsoft.Network/publicIPAddresses/%s", name)),
		Name:     ptr.To(name),
...
func TestListAllNotFound(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
...
func TestListAllInternalError(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
...
func TestListAllThrottle(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
...
func TestListAllWithListResponderError(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
