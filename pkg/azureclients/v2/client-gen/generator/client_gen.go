...
func (g Generator) Generate(ctx *genall.GenerationContext) error {
	...
	for _, root := range ctx.Roots {
		...
		markers.EachType(ctx.Collector, root, func(typeInfo *markers.TypeInfo) {
...
func generateClient(ctx *genall.GenerationContext, pkg *loader.Package, codeSnips []*bytes.Buffer, importList map[string]map[string]struct{}, headerText string) error {
	...
	for packageName, alias := range importList {
		if len(alias) == 0 {
			ImportTemplate.Execute(importWriter, &ImportStatement{Alias: "", Package: packageName})
		}
		for item := range alias {
			ImportTemplate.Execute(importWriter, &ImportStatement{Alias: item, Package: packageName})
		}
	}
	importWriter.Flush()
