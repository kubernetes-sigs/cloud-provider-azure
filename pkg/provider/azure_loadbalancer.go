...
func (az *Cloud) existsPip(ctx context.Context, clusterName string, service *v1.Service) bool {
	...
	existsPipSingleStack := func(isIPv6 bool) bool {
		...
		_, err = az.pipRepo.Get(ctx, pipResourceGroup, pipName, azcache.CacheReadTypeDefault)
		if err != nil {
			return false
		}
		return true
...
func (az *Cloud) GetLoadBalancer(ctx context.Context, clusterName string, service *v1.Service) (status *v1.LoadBalancerStatus, exists bool, err error) {
	...
	existingLBs, err := az.lbRepo.ListByResourceGroup(ctx, az.getLoadBalancerResourceGroup())
	if err != nil {
...
func (az *Cloud) EnsureLoadBalancerDeleted(ctx context.Context, clusterName string, service *v1.Service) (err error) {
	...
	lb, _, _, lbIPsPrimaryPIPs, _, err := az.getServiceLoadBalancer(ctx, service, clusterName, nil, false, nil)
...
func (az *Cloud) removeFrontendIPConfigurationFromLoadBalancer(ctx context.Context, lb *armnetwork.LoadBalancer, existingLBs []*armnetwork.LoadBalancer, fips []*armnetwork.FrontendIPConfiguration, clusterName string, service *v1.Service) (string, error) {
	...
	if len(fipConfigs) == 0 {
		...
		_, err := az.lbRepo.CreateOrUpdate(ctx, az.getLoadBalancerResourceGroup(), lb)
		if err != nil {
...
func (az *Cloud) cleanOrphanedLoadBalancer(ctx context.Context, lb *armnetwork.LoadBalancer, existingLBs []*armnetwork.LoadBalancer, service *v1.Service, clusterName string) error {
	...
	if isBackendPoolPreConfigured {
		...
		if deleteErr := az.safeDeleteLoadBalancer(ctx, lb, clusterName, vmSetName, service); deleteErr != nil {
			...
			if err := az.lbRepo.Delete(ctx, az.getLoadBalancerResourceGroup(), lbName); err != nil {
...
func (az *Cloud) safeDeleteLoadBalancer(ctx context.Context, lb *armnetwork.LoadBalancer, _, vmSetName string, service *v1.Service) *retry.Error {
	lbBackendPoolIDsToDelete := []string{}
	...
	if err := az.lbRepo.Delete(ctx, az.getLoadBalancerResourceGroup(), ptr.Deref(lb.Name, "")); err != nil {
...
func (az *Cloud) getServiceLoadBalancer(
	...
	if existingLBs == nil || len(existingLBs) == 0 {
		lbs, err := az.lbRepo.ListByResourceGroup(ctx, az.getLoadBalancerResourceGroup())
		if err != nil {
			return nil, nil, nil, nil, false, err
		}
		existingLBs = lbs
	...
	if defaultLB == nil {
		...
		if az.HasExtendedLocation() {
			var typ = getExtendedLocationTypeFromString(az.ExtendedLocationType)
...
func (az *Cloud) selectLoadBalancer(ctx context.Context, clusterName string, service *v1.Service, existingLBs []*armnetwork.LoadBalancer, nodes []*v1.Node) (selectedLB *armnetwork.LoadBalancer, existsLb bool, err error) {
	...
	for _, currVMSetName := range *vmSetNames {
		...
		if !exists {
			...
			if az.UseStandardLoadBalancer() {
				loadBalancerSKU = armnetwork.LoadBalancerSKUNameStandard
			} else {
				loadBalancerSKU = armnetwork.LoadBalancerSKUNameBasic
			}
			selectedLB = &armnetwork.LoadBalancer{
				Name:       &currLBName,
				Location:   &az.Location,
				Properties: &armnetwork.LoadBalancerPropertiesFormat{},
				SKU:        &armnetwork.LoadBalancerSKU{Name: to.Ptr(loadBalancerSKU)},
			}
			if az.HasExtendedLocation() {
				var typ = getExtendedLocationTypeFromString(az.ExtendedLocationType)
	...
	for i := range lb.Properties.FrontendIPConfigurations {
		...
		if owns {
			...
			if isInternal {
				...
				pip, err := az.pipRepo.Get(ctx, az.getPublicIPAddressResourceGroup(service), pipName, azcache.CacheReadTypeDefault)
				if err != nil {
...
func (az *Cloud) ensurePublicIPExists(ctx context.Context, service *v1.Service, pipName string, domainNameLabel, clusterName string, shouldPIPExisted, foundDNSLabelAnnotation, isIPv6 bool) (*armnetwork.PublicIPAddress, error) {
	pipResourceGroup := az.getPublicIPAddressResourceGroup(service)
	pip, err := az.pipRepo.Get(ctx, pipResourceGroup, pipName, azcache.CacheReadTypeDefault)
	notFound := publicip.IsNotFound(err)
	if err != nil && !notFound {
		return nil, err
	}
	serviceName := getServiceName(service)
	...
	if !notFound {
		...
		if strings.EqualFold(getDomainNameLabel(pip), domainNameLabel) {
			if existingServiceName := getServiceFromPIPDNSTags(pip.Tags); existingServiceName != "" && strings.EqualFold(existingServiceName, serviceName) {
				...
				if changed {
					...
					_, err = az.pipRepo.CreateOrUpdate(ctx, pipResourceGroup, pip)
					if err != nil {
						return nil, err
					}
					pip, err = az.pipRepo.Get(ctx, pipResourceGroup, *pip.Name, azcache.CacheReadTypeForceRefresh)
					if err != nil {
						return nil, err
					}
				}
		...
		if az.UseStandardLoadBalancer() {
			...
			if !az.HasExtendedLocation() {
				...
				if len(zones) > 0 {
					pip.Zones = to.SliceOfPtrs(zones...)
				}
			}
		}
...
func (az *Cloud) reconcileIPSettings(pip *armnetwork.PublicIPAddress, service *v1.Service, isIPv6 bool) bool {
	...
	serviceName := getServiceName(service)
	if isIPv6 {
		if *pip.Properties.PublicIPAddressVersion != armnetwork.IPVersionIPv6 {
		...
		if az.UseStandardLoadBalancer() {
			...
			if *pip.Properties.PublicIPAllocationMethod != armnetwork.IPAllocationMethodStatic {
				pip.Properties.PublicIPAllocationMethod = to.Ptr(armnetwork.IPAllocationMethodStatic)
				changed = true
			}
		} else if *pip.Properties.PublicIPAllocationMethod != armnetwork.IPAllocationMethodDynamic {
			pip.Properties.PublicIPAllocationMethod = to.Ptr(armnetwork.IPAllocationMethodDynamic)
			changed = true
		}
	} else {
		if *pip.Properties.PublicIPAddressVersion != armnetwork.IPVersionIPv4 {
...
func sortIPTags(ipTags []*armnetwork.IPTag) {
	sort.Slice(ipTags, func(i, j int) bool {
		ipTag := ipTags
		if ptr.Deref(ipTag[i].IPTagType, "") == ptr.Deref(ipTag[j].IPTagType, "") {
			return ptr.Deref(ipTag[i].Tag, "") < ptr.Deref(ipTag[j].Tag, "")
		}
...
func (az *Cloud) isFrontendIPChanged(
	...
	pip, err := az.pipRepo.Get(ctx, pipRG, pipName, azcache.CacheReadTypeDefault)
	if err != nil {
		if publicip.IsNotFound(err) {
			return true, nil
		}
...
func (az *Cloud) isFrontendIPConfigUnsafeToDelete(
	...
	for _, outboundRule := range outboundRules {
		if outboundRule.Properties != nil {
...
func (az *Cloud) reconcileLoadBalancer(ctx context.Context, clusterName string, service *v1.Service, nodes []*v1.Node, wantLb bool) (*armnetwork.LoadBalancer, error) {
	...
	if dirtyLb && len(toDeleteConfigs) > 0 {
		...
		if len(lb.Properties.FrontendIPConfigurations) == 0 {
			...
			newLB, err := az.lbRepo.Get(ctx, az.getLoadBalancerResourceGroup(), lbName, azcache.CacheReadTypeDefault)
			if err != nil {
				klog.Errorf("reconcileLoadBalancer for service(%s): getAzureLoadBalancer(%s) failed: %v", serviceName, lbName, err)
				return nil, fmt.Errorf("reconcile loadbalancer: %w", err)
			}
...
func (az *Cloud) reconcileLBProbes(
	lb *armnetwork.LoadBalancer,
	service *v1.Service,
	serviceName string,
	wantLb bool,
...
func (az *Cloud) reconcileLBRules(
	lb *armnetwork.LoadBalancer,
	service *v1.Service,
	serviceName string,
	wantLb bool,
...
func (az *Cloud) reconcileFrontendIPConfigs(
	...
	if !wantLb {
		...
		if isInternal {
			...
			subnet, err = az.subnetRepo.Get(ctx, az.VnetResourceGroup, az.VnetName, *subnetName)
			if err != nil {
				return nil, toDeleteConfigs, false, err
			}
		}
		...
		addNewFIPOfService := func(isIPv6 bool) error {
			...
			if isInternal {
				...
				if isIPv6 {
					configProperties.PrivateIPAddressVersion = ptr.To(armnetwork.IPVersionIPv6)
				}
				...
				if loadBalancerIP != "" {
					...
					configProperties.PrivateIPAllocationMethod = ptr.To(armnetwork.IPAllocationMethodStatic)
					...
					configProperties.PrivateIPAllocationMethod = ptr.To(armnetwork.IPAllocationMethodDynamic)
				}
...
func (az *Cloud) getFrontendZones(
	...
	if !isFipChanged { // fetch zone information from API for new frontends
		...
		if az.UseStandardLoadBalancer() && len(zones) > 0 && !az.HasExtendedLocation() {
			fipConfig.Zones = to.SliceOfPtrs(zones...)
		}
	} else {
		if previousZone == nil { // keep the existing zone information for existing frontends
			...
			var vs []string
			for _, s := range previousZone {
				vs = append(vs, *s)
			}
			zoneStr := strings.Join(vs, ",")
...
func (az *Cloud) getExpectedLoadBalancingRulePropertiesForPort(
	...
	if service.Spec.SessionAffinity == v1.ServiceAffinityClientIP {
		loadDistribution = armnetwork.LoadDistributionSourceIP
	}
	...
	props := &armnetwork.LoadBalancingRulePropertiesFormat{
		Protocol:            to.Ptr(transportProto),
		...
		LoadDistribution:    to.Ptr(loadDistribution),
	...
	if transportProto == armnetwork.TransportProtocolTCP && az.UseStandardLoadBalancer() {
...
func (az *Cloud) getExpectedHAModeLoadBalancingRuleProperties(
	...
	props, err := az.getExpectedLoadBalancingRulePropertiesForPort(service, lbFrontendIPConfigID, lbBackendPoolID, v1.ServicePort{}, armnetwork.TransportProtocolAll)
	if err != nil {
...
func (az *Cloud) reconcileSecurityGroup(
	...
	)
	{
		var backendIPv4List, backendIPv6List []string
		{
			...
			lb, err := az.lbRepo.Get(ctx, az.getLoadBalancerResourceGroup(), lbName, azcache.CacheReadTypeDefault)
			if err != nil && !loadbalancer.IsNotFound(err) {
				return nil, fmt.Errorf("reconcile security group: %w", err)
			}
...
func shouldReleaseExistingOwnedPublicIP(
	...
	pipPropertiesFormat := existingPip.Properties
...
func (az *Cloud) reconcilePublicIP(ctx context.Context, pips []*armnetwork.PublicIPAddress, clusterName string, service *v1.Service, lbName string, wantLb, isIPv6 bool) (*armnetwork.PublicIPAddress, error) {
	...
	if lbName != "" {
		lb, err = az.lbRepo.Get(ctx, az.getLoadBalancerResourceGroup(), lbName, azcache.CacheReadTypeDefault)
		if err != nil && !loadbalancer.IsNotFound(err) {
			return nil, fmt.Errorf("reconcile public IP: %w", err)
		}
	}
	...
	for _, pip := range pipsToBeUpdated {
		...
		updateFuncs = append(updateFuncs, func() error {
			...
			_, err := az.pipRepo.CreateOrUpdate(ctx, pipResourceGroup, &pipCopy)
			return err
		})
	}
...
func (az *Cloud) getPublicIPUpdates(
	...
	for i := range pips {
		...
		if pip.Properties != nil && pip.Properties.PublicIPAddressVersion != nil {
			if (pip.Properties.PublicIPAddressVersion == to.Ptr(armnetwork.IPVersionIPv4) && isIPv6) ||
				(pip.Properties.PublicIPAddressVersion == to.Ptr(armnetwork.IPVersionIPv6) && !isIPv6) {
				continue
			}
		}
...
func (az *Cloud) safeDeletePublicIP(
	ctx context.Context,
	service *v1.Service,
	...
	if pip.Properties != nil &&
		...
		latestPIP, err := az.pipRepo.Get(ctx, pipResourceGroup, *pip.Name, azcache.CacheReadTypeForceRefresh)
		notFound := publicip.IsNotFound(err)
		if err != nil && !notFound {
			klog.Errorf("safeDeletePublicIP: failed to get latest public IP %s/%s: %s", pipResourceGroup, *pip.Name, err.Error())
			return err
		}
		if !notFound && latestPIP.Properties != nil &&
	...
	err := az.pipRepo.Delete(ctx, pipResourceGroup, pipName)
	if err != nil {
		return err
	}
...
func equalLoadBalancingRulePropertiesFormat(s *armnetwork.LoadBalancingRulePropertiesFormat, t *armnetwork.LoadBalancingRulePropertiesFormat, wantLB bool) bool {
	...
	properties := true
	if s.Protocol == to.Ptr(armnetwork.TransportProtocolTCP) {
		properties = ptr.Deref(s.EnableTCPReset, false) == ptr.Deref(t.EnableTCPReset, false)
	}
...
func serviceOwnsPublicIP(service *v1.Service, pip *armnetwork.PublicIPAddress, clusterName string) (bool, bool) {
	...
	isIPv6 := pip.Properties.PublicIPAddressVersion == to.Ptr(armnetwork.IPVersionIPv6)
	if pip.Tags != nil {
