...
func (az *Cloud) safeDeleteLoadBalancer(lb network.LoadBalancer, clusterName, vmSetName string, service *v1.Service) *retry.Error {
	...
	klog.V(2).Infof("safeDeleteLoadBalancer: deleting LB %s because the corresponding vmSet is supposed to be in the primary SLB", pointer.StringDeref(lb.Name, ""))
				...
				if backendPool.BackendAddressPoolPropertiesFormat != nil &&
					...
					for _, ipConfiguration := range *backendPool.BackendIPConfigurations {
						if ipConfiguration.ID != nil {
							result = append(result, pointer.StringDeref(ipConfiguration.ID, ""))
						}
					}
				}
			}
...
func (az *Cloud) reconcileSharedLoadBalancer(service *v1.Service, clusterName string, nodes []*v1.Node) ([]network.LoadBalancer, error) {
	...
	for i := len(existingLBs) - 1; i >= 0; i-- {
		...
		isInternalLB := strings.HasSuffix(pointer.StringDeref(lb.Name, ""), consts.InternalLoadBalancerNameSuffix)
	...
	for _, primarySLB := range primarySLBs {
		if primarySLB.LoadBalancerPropertiesFormat != nil &&
			...
			for i := 0; i < len(*primarySLB.BackendAddressPools); i++ {
				if strings.EqualFold(pointer.StringDeref((*primarySLB.BackendAddressPools)[i].Name, ""), lbBackendPoolName) {
					if az.isLBBackendPoolTypeNodeIPConfig() {
						...
						if err := az.LoadBalancerBackendPool.EnsureHostsInPool(service, nodes, "", "", clusterName, pointer.StringDeref(primarySLB.Name, ""), backendPool); err != nil {
							return nil, fmt.Errorf("reconcileSharedLoadBalancer: failed to EnsureHostsInPool: %w", err)
						}
	...
	for i, existingLB := range existingLBs {
		for _, primarySLB := range primarySLBs {
			if strings.EqualFold(pointer.StringDeref(existingLB.Name, ""), pointer.StringDeref(primarySLB.Name, "")) {
...
func (az *Cloud) buildHealthProbeRulesForPort(annotations map[string]string, port v1.ServicePort, lbrule string) (*network.Probe, error) {
	...
	if requestPath == nil {
		...
		port.AppProtocol = pointer.String(string(network.ProtocolTCP))
	}
	...
	if strings.EqualFold(string(properties.Protocol), string(network.ProtocolHTTPS)) || strings.EqualFold(string(properties.Protocol), string(network.ProtocolHTTP)) {
		if requestPath == nil {
			...
			requestPath = pointer.String(consts.HealthProbeDefaultRequestPath)
		}
...
func findSecurityRule(rules []network.SecurityRule, rule network.SecurityRule) bool {
	for _, existingRule := range rules {
		...
		if !allowsConsolidation(existingRule) && !allowsConsolidation(rule) {
			...
			if !SliceEqual(stringSlice(existingRule.DestinationAddressPrefixes), stringSlice(rule.DestinationAddressPrefixes)) {
				continue
			}
		}
