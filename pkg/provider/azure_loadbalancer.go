...
func (az *Cloud) reconcileService(ctx context.Context, clusterName string, service *v1.Service, nodes []*v1.Node) (*v1.LoadBalancerStatus, error) {
	...
	lbStatus, lbIPsNoAdditionalPIPs, fipConfigs, err := az.getServiceLoadBalancerStatus(service, lb)
	if err != nil {
	...
	serviceIPs := lbIPsNoAdditionalPIPs
...
func (az *Cloud) EnsureLoadBalancerDeleted(ctx context.Context, clusterName string, service *v1.Service) error {
	...
	_, _, lbIPsNoAdditionalPIPs, _, err := az.getServiceLoadBalancer(service, clusterName, nil, false, []network.LoadBalancer{})
	if err != nil && !retry.HasStatusForbiddenOrIgnoredError(err) {
		return err
	}
	serviceIPsToCleanup := lbIPsNoAdditionalPIPs
...
func (az *Cloud) cleanOrphanedLoadBalancer(lb *network.LoadBalancer, existingLBs []network.LoadBalancer, service *v1.Service, clusterName string) error {
	...
	if v4Enabled {
		lbBackendPoolIDsToDelete = append(lbBackendPoolIDsToDelete, lbBackendPoolIDs[IsIPv4])
	}
	if v6Enabled {
		lbBackendPoolIDsToDelete = append(lbBackendPoolIDsToDelete, lbBackendPoolIDs[IsIPv6])
	}
...
func (az *Cloud) getServiceLoadBalancer(service *v1.Service, clusterName string, nodes []*v1.Node, wantLb bool, existingLBs []network.LoadBalancer) (lb *network.LoadBalancer, status *v1.LoadBalancerStatus, lbIPsNoAdditionalPIPs []string, exists bool, err error) {
	...
	for i := range existingLBs {
		...
		status, lbIPsNoAdditionalPIPs, fipConfigs, err = az.getServiceLoadBalancerStatus(service, &existingLB)
		if err != nil {
			return nil, nil, nil, false, err
		}
		if status == nil {
		...
		klog.V(4).Infof("getServiceLoadBalancer(%s, %s, %v): current lb IPs: %q", service.Name, clusterName, wantLb, lbIPsNoAdditionalPIPs)
		...
		return &existingLB, status, lbIPsNoAdditionalPIPs, true, nil
	}
	...
	if wantLb && !az.useStandardLoadBalancer() {
		...
		return selectedLB, status, lbIPsNoAdditionalPIPs, exists, err
	}
