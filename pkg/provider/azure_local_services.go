...
func (updater *loadBalancerBackendPoolUpdater) process(ctx context.Context) {
	...
	for key, ops := range groups {
		...
		bp, err := updater.az.lbRepo.GetBackendPool(ctx, updater.az.ResourceGroup, lbName, poolName)
		if err != nil {
		...
		if changed {
			...
			_, err = updater.az.lbRepo.CreateOrUpdateBackendPool(
				ctx, updater.az.ResourceGroup, lbName, bp,
			)
			if err != nil {
				updater.processError(err, operationName, ops...)
				continue
...
func (updater *loadBalancerBackendPoolUpdater) processError(
	...
	if loadbalancer.IsBackendPoolNotFound(err) {
		klog.V(4).Infof("backend pool not found for operation %s, skip updating", operationName)
		return
	}

	updater.notify(newBatchOperationResult(operationName, false, err), operations...)
}
		...
		for _, lb := range lbs {
			...
			for _, bp := range lb.Properties.BackendAddressPools {
				...
				if localServiceOwnsBackendPool(getServiceName(svc), bpName) {
					if err := az.lbRepo.DeleteBackendPool(ctx, az.getLoadBalancerResourceGroup(), lbName, bpName); err != nil {
						return nil, err
					}
					changed = true
				}
