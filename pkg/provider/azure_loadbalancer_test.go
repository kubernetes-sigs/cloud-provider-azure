...
func TestGetLoadBalancer(t *testing.T) {
	...
	for _, c := range tests {
		t.Run(c.desc, func(t *testing.T) {
			...
			mockLBsClient.EXPECT().List(gomock.Any(), az.ResourceGroup).Return(c.existingLBs, nil)
...
func TestShouldUpdateLoadBalancer(t *testing.T) {
	...
	for _, test := range testCases {
		t.Run(test.desc, func(t *testing.T) {
			...
			mockVMSet.EXPECT().GetPrimaryVMSetName().Return(az.PrimaryAvailabilitySetName).MaxTimes(3)
			az.VMSet = mockVMSet
