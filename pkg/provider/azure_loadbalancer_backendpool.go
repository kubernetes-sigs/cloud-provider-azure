...
//go:generate sh -c "mockgen -destination=./azure_mock_loadbalancer_backendpool.go -source=./azure_loadbalancer_backendpool.go -package=provider BackendPool"
...
type BackendPool interface {
	...
	EnsureHostsInPool(
		...
		service *v1.Service,
		nodes []*v1.Node,
		backendPoolID, vmSetName, clusterName, lbName string,
	...
	CleanupVMSetFromBackendPoolByCondition(
		ctx context.Context,
		slb *armnetwork.LoadBalancer,
		service *v1.Service,
		nodes []*v1.Node,
		clusterName string,
		shouldRemoveVMSetFromSLB func(string) bool,
...
func (bc *backendPoolTypeNodeIPConfig) EnsureHostsInPool(
	ctx context.Context,
	service *v1.Service,
	nodes []*v1.Node,
	backendPoolID, vmSetName, _, _ string,
	_ *armnetwork.BackendAddressPool,
) error {
...
func (bc *backendPoolTypeNodeIPConfig) CleanupVMSetFromBackendPoolByCondition(
	ctx context.Context,
	slb *armnetwork.LoadBalancer,
	service *v1.Service,
	_ []*v1.Node,
			...
			if bp.Properties != nil {
				for i := len(bp.Properties.BackendIPConfigurations) - 1; i >= 0; i-- {
					...
					if shouldRemoveVMSetFromSLB(vmSetName) {
						...
						vmSetNameToBackendIPConfigurationsToBeDeleted[vmSetName] = append(
							vmSetNameToBackendIPConfigurationsToBeDeleted[vmSetName],
							interfaceIPConfigToBeDeleted,
						)
						bp.Properties.BackendIPConfigurations = append(
							bp.Properties.BackendIPConfigurations[:i],
							bp.Properties.BackendIPConfigurations[i+1:]...,
						)
					}
				}
			}
	...
	for vmSetName := range vmSetNameToBackendIPConfigurationsToBeDeleted {
		...
		if shouldRefreshLB {
			slb, err := bc.lbRepo.Get(ctx, bc.getLoadBalancerResourceGroup(), ptr.Deref(slb.Name, ""), cache.CacheReadTypeForceRefresh)
...
func (bc *backendPoolTypeNodeIPConfig) ReconcileBackendPools(
		...
		if found {
			...
			if bp.Properties != nil &&
				...
				if removeNodeIPAddressesFromBackendPool(bp, []string{}, true, false, false) {
					...
					if _, err := bc.lbRepo.CreateOrUpdateBackendPool(ctx, bc.getLoadBalancerResourceGroup(), lbName, bp); err != nil {
	...
	if backendPoolsUpdated {
		...
		var err error
		lb, err = bc.lbRepo.Get(ctx, bc.getLoadBalancerResourceGroup(), lbName, cache.CacheReadTypeForceRefresh)
		if err != nil && !loadbalancer.IsNotFound(err) {
	...
	return isBackendPoolPreConfigured, backendPoolsCreated, lb, nil
}
...
func (bc *backendPoolTypeNodeIPConfig) GetBackendPrivateIPs(
	ctx context.Context,
	clusterName string,
	service *v1.Service,
	lb *armnetwork.LoadBalancer,
...
func (bi *backendPoolTypeNodeIP) EnsureHostsInPool(
	ctx context.Context,
	service *v1.Service,
	nodes []*v1.Node,
	_, _, clusterName, lbName string,
	backendPool *armnetwork.BackendAddressPool,
) error {
	...
	if changed {
		...
		if _, err := bi.lbRepo.CreateOrUpdateBackendPool(ctx, bi.getLoadBalancerResourceGroup(), lbName, backendPool); err != nil {
...
func (bi *backendPoolTypeNodeIP) CleanupVMSetFromBackendPoolByCondition(
	ctx context.Context,
	slb *armnetwork.LoadBalancer,
	_ *v1.Service,
	nodes []*v1.Node,
				...
				for i := len(bp.Properties.LoadBalancerBackendAddresses) - 1; i >= 0; i-- {
					if bp.Properties.LoadBalancerBackendAddresses[i].Properties != nil &&
						...
						bp.Properties.LoadBalancerBackendAddresses = append(
							bp.Properties.LoadBalancerBackendAddresses[:i],
							bp.Properties.LoadBalancerBackendAddresses[i+1:]...,
						)
	...
	for isIPv6 := range updatedPrivateIPs {
		...
		for _, backendAddressPool := range slb.Properties.BackendAddressPools {
			if strings.EqualFold(lbBackendPoolNames[isIPv6], ptr.Deref(backendAddressPool.Name, "")) {
				if _, err := bi.lbRepo.CreateOrUpdateBackendPool(
					ctx, bi.getLoadBalancerResourceGroup(), ptr.Deref(slb.Name, ""), backendAddressPool,
				); err != nil {
...
func (bi *backendPoolTypeNodeIP) ReconcileBackendPools(
	ctx context.Context,
	clusterName string,
	service *v1.Service,
	lb *armnetwork.LoadBalancer,
	...
	if !isBackendPoolPreConfigured {
		...
		for _, i := range bpIdxes {
			...
			if updated {
				...
				if _, err := bi.lbRepo.CreateOrUpdateBackendPool(ctx, bi.getLoadBalancerResourceGroup(), lbName, bp); err != nil {
	...
	if shouldRefreshLB {
		...
		lb, err = bi.lbRepo.Get(ctx, bi.getLoadBalancerResourceGroup(), lbName, cache.CacheReadTypeForceRefresh)
		if err != nil && !loadbalancer.IsNotFound(err) {
...
func (bi *backendPoolTypeNodeIP) GetBackendPrivateIPs(
	_ context.Context,
	clusterName string,
	service *v1.Service,
	lb *armnetwork.LoadBalancer,
...
func newBackendPool(
	lb *armnetwork.LoadBalancer,
	isBackendPoolPreConfigured bool,
	preConfiguredBackendPoolLoadBalancerTypes,
	serviceName, lbBackendPoolName string,
) bool {
	if isBackendPoolPreConfigured {
...
func hasIPAddressInBackendPool(backendPool *armnetwork.BackendAddressPool, ipAddress string) bool {
	for _, address := range backendPool.Properties.LoadBalancerBackendAddresses {
