		...
		for _, tt := range tests {
			t.Run(tt.Name, func(t *testing.T) {
				...
				addNode := func(ip string) {
					if _, ok := az.nodePrivateIPs[ip]; ok {
						return
					}
					az.nodePrivateIPs[ip] = utilsets.NewString(ip)
					runtimeObjects = append(runtimeObjects, &v1.Node{
						ObjectMeta: metav1.ObjectMeta{Name: ip},
						Status:     v1.NodeStatus{Addresses: []v1.NodeAddress{{Type: v1.NodeInternalIP, Address: ip}}},
					})
				}
				addNode(backendNodeIP)
				...
				for _, cfg := range tt.MultiSLBConfigs {
					for _, ip := range cfg.ActiveNodes.UnsortedList() {
						addNode(ip)
					}
				}
				...
				if tt.ExistingBackendPrivateIPs != nil {
					...
					for _, cfg := range tt.MultiSLBConfigs {
						if cfg.ActiveServices.Has(reconciledKey) {
							backendPrivateIPs = cfg.ActiveNodes.UnsortedList()
							break
						}
					}
				}
...
func TestFilterServicesSharingDestinations(t *testing.T) {
	...
	var (
		...
		others = []*v1.Service{
			...
			floatingIPDisabledWithSharedAdditionalIP, notProvisioned,
		}
	)
