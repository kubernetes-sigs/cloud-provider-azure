...
func getNodePrivateIPAddress(service *v1.Service, node *v1.Node, isIPv6 bool) string {
...
func ifIPFamiliesEnabled1(svc *v1.Service) (v4Enabled bool, v6Enabled bool) {
...
// ifFIPIPv6 checks if the frontend IP configuration is of IPv6.
func (az *Cloud) ifFIPIPv6(service *v1.Service, fip *network.FrontendIPConfiguration, pips *[]network.PublicIPAddress, isInternal bool) (isIPv6 bool, err error) {
	if err := az.ensurePIP(service, az.ResourceGroup, pips); err != nil {
		return false, fmt.Errorf("failed to ensure PIP is refreshed: %v", err)
	}
	if isInternal {
