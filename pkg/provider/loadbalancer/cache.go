...
func NewCache(
	client loadbalancerclient.Interface,
	cacheTTL time.Duration,
	disableAPICallCache bool,
) (cache.Resource, error) {
	getter := func(ctx context.Context, key string) (interface{}, error) {
		resourceGroup, loadBalancerName := parseCacheKey(key)

		rt, err := client.Get(ctx, resourceGroup, loadBalancerName, nil)
...
func getCacheKey(resourceGroup, loadBalancerName string) string {
	return fmt.Sprintf("%s/%s", resourceGroup, loadBalancerName)
}
