...
func (az *Cloud) updateNodeCaches(prevNode, newNode *v1.Node) {
	...
	if prevNode != nil {
		...
		if _, hasExcludeBalancerLabel := prevNode.ObjectMeta.Labels[consts.LabelNodeRoleExcludeBalancer]; hasExcludeBalancerLabel {
			az.excludeLoadBalancerNodes.Delete(prevNode.ObjectMeta.Name)
		}
	}
	...
	if newNode != nil {
		...
		if _, hasExcludeBalancerLabel := newNode.ObjectMeta.Labels[consts.LabelNodeRoleExcludeBalancer]; hasExcludeBalancerLabel {
			az.excludeLoadBalancerNodes.Insert(newNode.ObjectMeta.Name)
		}
	}
}
