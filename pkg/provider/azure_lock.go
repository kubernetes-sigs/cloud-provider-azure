...
func (l *AzureResourceLocker) acquireLease(
	...
	if !hasExpired(lease) {
		...
		if !strings.EqualFold(prevHolder, holder) {
			errMsg := "lease has not expired yet, another component such as aks rp may be processing another request; this automatically recovers after the lease expires"
