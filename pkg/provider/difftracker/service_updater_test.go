...
func TestServiceUpdaterProcessBatchFlow(t *testing.T) {
	dt := &DiffTracker{
		...
		pendingServiceOps: map[string]*ServiceOperationState{
			"service-1": {ServiceUID: "service-1", IsInbound: true, State: StateNotStarted, RetryCount: 0},
			"service-2": {ServiceUID: "service-2", IsInbound: true, State: StateCreationInProgress, RetryCount: 0},
			"service-3": {ServiceUID: "service-3", IsInbound: true, State: StateCreated, RetryCount: 0},
			"service-4": {ServiceUID: "service-4", IsInbound: true, State: StateDeletionPending, RetryCount: 0},
			"service-5": {ServiceUID: "service-5", IsInbound: true, State: StateDeletionInProgress, RetryCount: 0},
		},
