...
func TestReconcilePrivateLinkService(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedPLS:       &armnetwork.PrivateLinkService{Name: pointer.String("pls-fipConfig")},
		},
		{
			...
			existingPLSList:   []*armnetwork.PrivateLinkService{},
			expectedPLSCreate: true,
			expectedPLS:       &armnetwork.PrivateLinkService{Name: pointer.String("testpls")},
		},
		{
			...
			existingPLSList: []*armnetwork.PrivateLinkService{
				{
					...
					Properties: &armnetwork.PrivateLinkServiceProperties{
						LoadBalancerFrontendIPConfigurations: []*armnetwork.FrontendIPConfiguration{{ID: pointer.String("fipConfigID")}},
						IPConfigurations: []*armnetwork.PrivateLinkServiceIPConfiguration{
							{
								Properties: &armnetwork.PrivateLinkServiceIPConfigurationProperties{
									...
									Subnet:                    &armnetwork.Subnet{ID: pointer.String("subnetID")},
			...
			existingPLSList: []*armnetwork.PrivateLinkService{
				{
					Name: pointer.String("testpls"),
					Properties: &armnetwork.PrivateLinkServiceProperties{
						LoadBalancerFrontendIPConfigurations: []*armnetwork.FrontendIPConfiguration{{ID: pointer.String("fipConfigID2")}},
					},
				},
			},
		},
			...
			existingIPConfigs: []*armnetwork.PrivateLinkServiceIPConfiguration{
				{
					...
					Properties: &armnetwork.PrivateLinkServiceIPConfigurationProperties{
						PrivateIPAllocationMethod: to.Ptr(armnetwork.IPAllocationMethodDynamic),
						Subnet:                    &armnetwork.Subnet{ID: pointer.String("subnetID1")},
						Primary:                   pointer.Bool(true),
						PrivateIPAddressVersion:   to.Ptr(armnetwork.IPVersionIPv4),
					},
				},
