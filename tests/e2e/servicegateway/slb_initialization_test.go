...
func getCloudProviderConfig(ctx context.Context, ccmClient *CCMClusterClient) (map[string]interface{}, error) {
...
func updateCloudProviderConfig(ctx context.Context, ccmClient *CCMClusterClient, config map[string]interface{}) error {
...
func getOriginalCloudProviderConfigJSON(ctx context.Context, ccmClient *CCMClusterClient) ([]byte, error) {
	secret, err := ccmClient.ClientSet.CoreV1().Secrets(ccmClient.Namespace).Get(
		ctx,
		cloudProviderConfigSecret,
		metav1.GetOptions{},
...
func restoreCloudProviderConfig(ctx context.Context, ccmClient *CCMClusterClient, originalJSON []byte) error {
...
func updateCCMImage(ctx context.Context, ccmClient *CCMClusterClient, image string) error {
...
func getCCMCurrentImage(ctx context.Context, ccmClient *CCMClusterClient) (string, error) {
	deployment, err := ccmClient.ClientSet.AppsV1().Deployments(ccmClient.Namespace).Get(
		ctx,
		ccmDeploymentName,
		metav1.GetOptions{},
...
func scaleCCMDeployment(ctx context.Context, ccmClient *CCMClusterClient, replicas int32) error {
...
func waitForCCMFullyDown(ctx context.Context, ccmClient *CCMClusterClient, timeout time.Duration) error {
...
func waitForCCMFullyUp(ctx context.Context, ccmClient *CCMClusterClient, timeout time.Duration) error {
...
var _ = Describe("Container Load Balancer Initialization Tests", Label(slbTestLabel, slbInitTestLabel), func() {
	...
	BeforeEach(func() {
		...
		if !IsCCMClusterConfigured() {
			Skip(fmt.Sprintf("Skipping CCM initialization tests: %s environment variable not set", CCMKubeconfigEnvVar))
		}
	...
	It("should reconcile 5 LoadBalancer services added during CCM downtime", func() {
		...
		err = waitForCCMFullyDown(ctx, ccmClient, CCMRecoveryTimeout)
		Expect(err).NotTo(HaveOccurred())
		...
		err = waitForCCMFullyUp(ctx, ccmClient, CCMRecoveryTimeout)
		Expect(err).NotTo(HaveOccurred())
