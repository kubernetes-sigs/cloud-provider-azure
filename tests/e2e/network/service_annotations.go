...
var _ = Describe("Service with annotation", Label(utils.TestSuiteLabelServiceAnnotation), func() {
	...
	It("should update azure-pip-ip-tags in-place without recreating the PIP", func() {
		if !strings.EqualFold(os.Getenv(utils.LoadBalancerSKUEnv), string(armnetwork.LoadBalancerSKUNameStandard)) {
		...
		By("Creating a service with azure-pip-ip-tags annotation")
		annotation := map[string]string{
		...
		expectedInitialIPTags := []*armnetwork.IPTag{{
			IPTagType: ptr.To("RoutingPreference"),
			Tag:       ptr.To("Internet"),
		}}
		...
		By("Verifying the IP tags were cleared in place")
		err = waitComparePIPIPTags(tc, []*armnetwork.IPTag{}, recordedName)
		Expect(err).NotTo(HaveOccurred())

		By("Verifying the PIP identity is preserved")
