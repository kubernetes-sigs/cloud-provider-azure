...
[Needs rebase](#details-needs-rebase) directive, whose `Stop=yes` ends the
current triage, and budgeted act-stage `Stop=no` actions that post a comment,
push, or trigger a CI rerun. The budgeted act-stage actions are
...
comment. After all retry-budgeted act-stage non-final actions taken this triage
...
[Public-IP quota e2e](#details-public-ip-quota-e2e) reruns alone do not consume
an attempt. A rebase directive causes no separate summary because its comment
