...
func Run(ctx context.Context, c *cloudcontrollerconfig.CompletedConfig, h *controllerhealthz.MutableHealthzHandler) error {
	...
	if err := startControllers(c, ctx.Done(), cloud, newControllerInitializers(), h); err != nil {
...
func startControllers(c *cloudcontrollerconfig.CompletedConfig, stopCh <-chan struct{}, cloud cloudprovider.Interface, controllers map[string]initFunc, healthzHandler *controllerhealthz.MutableHealthzHandler) error {
	...
	for controllerName, initFn := range controllers {
		...
		ctrl, started, err := initFn(c, cloud, stopCh)
		if err != nil {
