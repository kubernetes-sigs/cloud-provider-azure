...
| nodeInstanceNotFoundGracePeriodInSeconds                   | Grace period, measured from the node creation timestamp, during which a newly registered node whose backing VM/VMSS instance is not yet visible in ARM is still reported as existing.                            | Optional. Integer value, default to 0, which disables the grace period                                                                          |
