...
| loadBalancerBackendPoolUpdateMaxRetries                    | Maximum number of retries for the local service backend pool updater in multi-slb mode, default to 3. Set to 0 to disable.                                                                                        | Optional. Supported since v1.34.14, v1.35.9, and v1.36.5                                                                              |
...
| nodeInstanceNotFoundGracePeriodInSeconds                   | Grace period, measured from the node creation timestamp, during which a newly registered node whose backing VM/VMSS instance is not yet visible in ARM is still reported as existing.  Integer value, default to 0, which disables the grace period.                       | Optional. Supported since v1.33.15, v1.34.12, v1.35.7 and v1.36.3 |
